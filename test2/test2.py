...
csv_input = pd.read_csv("phrase2phrase_ver10_kai.csv")
for i in range(len(csv_input)):
    #for i in range(10):
    ongen = csv_input['ongen'][i]
    
    mp3_name = str(ongen) + ".mp3"
...
ssml_text="<speak>"
    ...
    for j in range(len(text)):
        ...
        if j!=int(rhyme_pos[0]) and flag==0:#1:強調部に入るまで,強調部出てから
            ssml_text = ssml_text + '<amazon:effect phonation="soft" vocal-tract-length="+5%"><prosody volume="-5dB" >' + text[j]
            flag=1
        # print("loop1")
        ...
        # print("loop2")
            ...
            ssml_text = ssml_text + '<amazon:effect vocal-tract-length="+10%"><prosody volume="+3dB" rate="88%">' + text[j]
            flag = 2
        # print("loop3")
        elif j==int(rhyme_pos[0]) and flag==1:#4:強調部に入る
            ssml_text = ssml_text + '</prosody></amazon:effect><amazon:effect vocal-tract-length="+10%"><prosody volume="+3dB" rate="88%">' + text[j]
            flag = 2
        # print("loop4")
